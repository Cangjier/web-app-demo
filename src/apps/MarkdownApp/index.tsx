...
export interface IMarkdownItemProps {
...
export interface IMarkdownItemRef {
...
export const MarkdownItem = forwardRef<IMarkdownItemRef, IMarkdownItemProps>((props, ref) => {
    let { items, item, path, index, data, updateData, currentTab, onTabChange } = props;
    if (typeof (item) == 'string') {
        if (item.startsWith('# ')) {
            return <MarkdownItem items={items} item={{
                type: '#', text: item.substring(2)
            }} path={path} index={index} data={data} updateData={updateData} currentTab={currentTab} onTabChange={onTabChange} />
        }
        ...
        return <Card key={key}>
            <Flex direction='column'>
                {item.children?.map((subItem, subIndex) => {
                    return <MarkdownItem items={item.children ?? []} item={subItem} path={key} index={subIndex} data={data} updateData={updateData} currentTab={currentTab} onTabChange={onTabChange} />
                })}
            </Flex>
        </Card>
    }
        ...
        for (let itemKey of item.tableOptions?.keys ?? []) {
            if (typeof itemKey == "string") {
                ...
                columns.push({
                    ...
                    width: itemKeyObject.columnWidth ?? "16em",
                    render: (recordText, record, recordIndex) => {
...
export const MarkdownApp = forwardRef<IMarkdownAppRef, IMarkdownAppProps>((props, ref) => {
    ...
    const self = useRef<IMarkdownAppRef>({
        ...
        renderItem: (items, item, path, index) => {
            return <MarkdownItem items={items} item={item} path={path} index={index} data={data} updateData={updateData} currentTab={props.currentTab} onTabChange={props.onTabChange} />
        },
        clearData: () => {
            updateData({});
        }
    ...
    return <Flex style={{
        ...
        {props.markdownLines?.map((item, itemIndex) => {
            return <MarkdownItem items={props.markdownLines ?? []} item={item} path={getMarkdownLineText(item)} index={itemIndex} data={data} updateData={updateData} currentTab={props.currentTab} onTabChange={props.onTabChange} />
        })}
    </Flex>
});